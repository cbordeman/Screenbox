...
namespace VLC.Net.Core.Services
{
    ...
    public sealed class LibraryService : ILibraryService
    {
        ...
        private bool UseIndexer => settingsService.UseIndexer;
        private bool SearchRemovableStorage => settingsService.SearchRemovableStorage && SystemInformation.IsXbox;
        ...
        private readonly AlbumViewModelFactory albumFactory;
        private readonly ArtistViewModelFactory artistFactory;
        private readonly DispatcherQueueTimer musicRefreshTimer;
        private readonly DispatcherQueueTimer videosRefreshTimer;
        ...
        private List<MediaViewModel> songs;
        private List<MediaViewModel> videos;
        private CancellationTokenSource? musicFetchCts;
        private CancellationTokenSource? videosFetchCts;
            ...
            MediaViewModelFactory mediaFactory, AlbumViewModelFactory albumFactory, ArtistViewModelFactory artistFactory)
        {
            ...
            musicRefreshTimer = dispatcherQueue.CreateTimer();
            videosRefreshTimer = dispatcherQueue.CreateTimer();
        ...
        {
            return new MusicLibraryFetchResult(songs.AsReadOnly(), albumFactory.AllAlbums.ToList(), artistFactory.AllArtists.ToList(),
        ...
        private async Task CacheSongsAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                await filesService.SaveToDiskAsync(ApplicationData.Current.LocalFolder, SongsCacheFileName, libraryCache);
            }
            catch (Exception)
            {
                // ignored
        ...
        private async Task CacheVideosAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                await filesService.SaveToDiskAsync(ApplicationData.Current.LocalFolder, VideoCacheFileName, libraryCache);
            }
            catch (Exception)
            {
                // ignored
        ...
        private async Task<PersistentStorageLibrary?> LoadStorageLibraryCacheAsync(string fileName)
        {
            try
            {
                return await filesService.LoadFromDiskAsync<PersistentStorageLibrary>(
        ...
        private List<MediaViewModel> GetMediaFromCache(PersistentStorageLibrary libraryCache)
        {
            ...
            {
                MediaViewModel media = mediaFactory.GetSingleton(new Uri(record.Path));
        ...
        private async Task FetchMusicCancelableAsync(bool useCache, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (songs != this.songs)
                {
                    ...
                    this.songs.ForEach(song => song.IsFromLibrary = false);
                }
                ...
                foreach (MediaViewModel song in songs)
                {
                    ...
                    {
                        song.UpdateAlbum(albumFactory);
                        song.UpdateArtists(artistFactory);
                    }
                    else
                    {
                        await song.LoadDetailsAsync(filesService);
                        cancellationToken.ThrowIfCancellationRequested();
                        song.UpdateAlbum(albumFactory);
                        song.UpdateArtists(artistFactory);
                    }
        ...
        private async Task FetchVideosCancelableAsync(bool useCache, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                foreach (MediaViewModel video in videos)
                {
                    ...
                    if (!hasCache)
                    {
                        await video.LoadDetailsAsync(filesService);
                        cancellationToken.ThrowIfCancellationRequested();
                    }
                }
        ...
        {
            List<MediaViewModel> outdatedSongs = songs.Where(song => !song.IsFromLibrary).ToList();
            ...
            albumFactory.Compact();
            artistFactory.Compact();
        }
        ...
        private async Task<List<MediaViewModel>> FetchMediaFromStorage(StorageFileQueryResult queryResult, uint fetchIndex, uint batchSize = 50)
        {
            ...
            List<MediaViewModel> mediaBatch = files.Select(mediaFactory.GetSingleton).ToList();
            return mediaBatch;
        }
            ...
            foreach (StorageLibraryChange change in changeBatch)
            {
                ...
                switch (change.ChangeType)
                {
                    ...
                    case StorageLibraryChangeType.MovedIntoLibrary:
                        ...
                        mediaList.Add(mediaFactory.GetSingleton(file));
                        break;
                    ...
                    case StorageLibraryChangeType.MovedOrRenamed:
                        ...
                        var newMedia = mediaFactory.GetSingleton(file);
                        if (existing != null)
                        {
        ...
        private void OnVideosLibraryContentChanged(object sender, object args)
        {
            ...
            videosRefreshTimer.Debounce(FetchAction, TimeSpan.FromMilliseconds(1000));
        }
        ...
        private void OnMusicLibraryContentChanged(object sender, object args)
        {
            ...
            // Delay fetch due to query result not yet updated at this time
            musicRefreshTimer.Debounce(FetchAction, TimeSpan.FromMilliseconds(1000));
        }
        ...
        private void OnPortableStorageDeviceChanged(DeviceWatcher sender, DeviceInformationUpdate args)
        {
            ...
            storageDeviceRefreshTimer.Debounce(FetchAction, TimeSpan.FromMilliseconds(1000));
        }

        private void StartPortableStorageDeviceWatcher()
        {
            if (portableStorageDeviceWatcher?.Status is DeviceWatcherStatus.Created or DeviceWatcherStatus.Stopped)
            {
