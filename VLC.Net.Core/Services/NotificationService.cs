...
namespace VLC.Net.Core.Services
{
    public sealed class NotificationService : INotificationService
    {
        ...
        private readonly Func<IVlcLoginDialog> vlcLoginDialogFactory;

        public NotificationService(Func<IVlcLoginDialog> vlcLoginDialogFactory)
        {
            this.vlcLoginDialogFactory = vlcLoginDialogFactory;
        }
        ...
        private Task UpdateProgress(Dialog dialog, float position, string? text) =>
            DisplayProgress(dialog, progressTitle, text, false, position, null, CancellationToken.None);
        ...
        {
            if (token.IsCancellationRequested) return;
            IVlcLoginDialog loginDialog = vlcLoginDialogFactory();
